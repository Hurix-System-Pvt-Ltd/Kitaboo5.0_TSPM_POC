...
public class CircularProgressButton extends Button {
	...
	private boolean mShowProgressOnly;

	private enum State {
		PROGRESS, IDLE, COMPLETE, ERROR, PAUSE, UPDATE
	}
	...
	private void initPauseStateDrawable() {
		int colorPressed = getPressedColor(mPauseColorState);

		StrokeGradientDrawable drawablePressed = createDrawable(colorPressed);
		mPauseStateDrawable = new StateListDrawable();

		mPauseStateDrawable.addState(new int[]{android.R.attr.state_pressed}, drawablePressed.getGradientDrawable());
		mPauseStateDrawable.addState(StateSet.WILD_CARD, background.getGradientDrawable());
	}
	...
	private StrokeGradientDrawable createDrawable(int color) {
		GradientDrawable drawable = (GradientDrawable) getResources().getDrawable(R.drawable.morph_button_background).mutate();
	...
	private void initAttributes(Context context, AttributeSet attributeSet) {
		...
		try {
			...
			mPauseText = attr.getString(R.styleable.CircularProgressButton_cpb_textPause);
			...
			mShowProgressOnly = attr.getBoolean(R.styleable.CircularProgressButton_cpb_showProgressOnly, false);
			
			
			int blue = getColor(R.color.kitaboo_main_color);
            ...
            /*int colorstate[][] = new int[][]{{0,0}, {0,0}};
            int color[] = new int[] {Color.parseColor(UserController.getInstance(context).getUserSettings()
                    .get_bookshelf_btnLaunch_background())};
                    new ColorStateList(colorstate, color); //*/
			...
			int pauseStateSelector = attr.getResourceId(R.styleable.CircularProgressButton_cpb_selectorPaused,
					R.color.pause_state_selector);
			mPauseColorState = getResources().getColorStateList(pauseStateSelector);

			int updateStateSelector = attr.getResourceId(R.styleable.CircularProgressButton_cpb_selectorUpdated,
					R.color.update_state_selector);
			mUpdateColorState = getResources().getColorStateList(updateStateSelector);
	...
	private void drawProgress(Canvas canvas) {
		...
		float sweepAngle = (360f / COMPLETE_STATE_PROGRESS) * mProgress;
	...
	private MorphingAnimation createMorphing(boolean isInstant) {
		...
		if (mConfigurationChanged || isInstant) {
	...
	private OnAnimationStartEndListener mProgressStateListener = new OnAnimationStartEndListener() {
		@Override
	...
	private void morphIdleToComplete() {
		MorphingAnimation animation = createMorphing(true);
	...
	private void morphPauseToComplete() {
		MorphingAnimation animation = createMorphing(false);

		animation.setFromColor(getNormalColor(mPauseColorState));
		animation.setToColor(getNormalColor(mCompleteColorState));

		animation.setFromStrokeColor(getNormalColor(mPauseColorState));
		animation.setToStrokeColor(getNormalColor(mCompleteColorState));
	...
	private void morphUpdateToComplete() {
		MorphingAnimation animation = createMorphing(false);
	...
	private OnAnimationStartEndListener mCompleteStateListener = new OnAnimationStartEndListener() {
	...
	private void morphCompleteToUpdate() {
		MorphingAnimation animation = createMorphing(false);
	...
	private void morphUpdateToIdle() {
		MorphingAnimation animation = createMorphing(false);
	...
	private void morphErrorToUpdate() {
		MorphingAnimation animation = createMorphing(false);
	...
	private void morphPauseToUpdate() {
		MorphingAnimation animation = createMorphing(false);
	...
	private OnAnimationStartEndListener mIdleStateListener = new OnAnimationStartEndListener() {
	...
	private void morphUpdateToError() 
	{
		MorphingAnimation animation = createMorphing(false);
	...
	private void morphPauseToError() 
	{
		MorphingAnimation animation = createMorphing(false);
	...
	private void morphIdleToUpdate() {
		MorphingAnimation animation = createMorphing(false);
	...
	private OnAnimationStartEndListener mUpdateStateListener = new OnAnimationStartEndListener() {
...
//			setText(mUpdateText);
	...
	private void morphIdleToPause() {
		MorphingAnimation animation = createMorphing(false);

		animation.setFromColor(getNormalColor(mIdleColorState));
		animation.setToColor(getNormalColor(mPauseColorState));

		animation.setFromStrokeColor(getNormalColor(mIdleColorState));
		animation.setToStrokeColor(getNormalColor(mPauseColorState));
	...
	private void morphCompleteToPause() {
		MorphingAnimation animation = createMorphing(false);
	...
	private void morphUpdateToPause() {
		MorphingAnimation animation = createMorphing(false);
	...
	private void morphErrorToPause() {
		MorphingAnimation animation = createMorphing(false);
	...
	private OnAnimationStartEndListener mErrorStateListener = new OnAnimationStartEndListener() {
	...
	private OnAnimationStartEndListener mPauseStateListener = new OnAnimationStartEndListener() {
	...
	private void morphProgressToIdle() {
		...
		animation.setFromStrokeColor(mColorIndicator);
		animation.setToStrokeColor(getNormalColor(mIdleColorState));
		animation.setListener(new OnAnimationStartEndListener() {
...
//		setPadding(0, 0, 0, 0);
	...
	public void setProgress(float progress) {
		...
		if (mProgress >= COMPLETE_STATE_PROGRESS) 
		{
			if (mState == State.PROGRESS) 
			{
			...
			{
				morphPauseToComplete();
			}
			else if(mState == State.UPDATE)
			{
				morphUpdateToComplete();
			}
			else if(mState == State.ERROR)
			{
			...
			if (mState == State.IDLE || mState == State.PAUSE || mState == State.ERROR || mState == State.UPDATE) {
				morphToProgress();
			} else if (mState == State.PROGRESS) {
				invalidate();
			}
		...
		else if (mProgress == UPDATE_STATE_PROGRESS) 
		{
			if (mState == State.IDLE) {
				morphIdleToUpdate();
			}else if (mState == State.PROGRESS) {
