...
public class ServiceHandler extends KitabooServiceAdapter {
     ...
     * @param userid
     * @param bookVersion
     * @param paginatedugcarray
     */
    public void sendSaveUGCRequest(final long bookid, final long userid, final String bookVersion, final JSONArray paginatedugcarray, int position) {
        String timestamp = getLastSyncedDateTime(bookid, userid);
        timestamp = Utils.escapeString(timestamp);

        super.backgroundSaveUGC(bookid,
                bookVersion, userid, PlayerController.getInstance(mContext).getToken(), timestamp, false, paginatedugcarray, new IServiceResponseListener() {
                    @Override
                    public void requestCompleted(IServiceResponse response) {
                        Log.e("SaveUGCSyncRequest : ", "sucess");
                        ...
                        if(position == 0) {
                            ...
                            if (mPagetrackdata != null && !mPagetrackdata.isEmpty()) {
                                savePageTrackingData(PlayerController.getInstance(mContext).getToken(), bookid, userid, mPagetrackdata);
                            }
    ...
    public void sendServicesOnScheduler(final long bookid, final long userid,
                                        final String bookVersion, String pagetrackdata) {
       ...
       /* if (ugcDataLength > 0) {
            ...
            if (mPagetrackdata !=null && !mPagetrackdata.isEmpty()) {
                savePageTrackingData(PlayerController.getInstance(mContext).getToken(), bookid, userid, mPagetrackdata);
               /* JSONObject obj = nnull;
                try {
                    obj = new JSONObject(mPagetrackdata);
                    JSONArray pageArr = obj.getJSONArray("trackings");
                    JSONObject obj2 = new JSONObject(pageArr.get(0).toString());
                    if (obj2.has("pageTracking")) {
                        JSONArray array = obj2.getJSONArray("pageTracking");
                        if (array.length() > 0) {
                            savePageTrackingData(UserController.getInstance(mContext).getUserVO().getToken(), bookid, userid, mPagetrackdata);
                            Log.d("Analytics", "savePageTrackingData");
                        } else {
                            *//*TODO new service*//*

                            saveLastPageAccessed(obj2.getString("lastPageFolio"), bookid);
                        }
                    }
  ...
  /*  public void sendFetchUGCRequest(final long bookid, final String accountType, final long userid, final String bookVersion, final IServiceResponseListener _iServiceResponse) {
        ...
        super.backgroundFetchUGC(userid, bookid,
                KitabooSDKModel.getInstance().getUserToken(), timestamp, bookVersion, new IServiceResponseListener() {
                    ...
                    public void requestCompleted(IServiceResponse response) {
                        *//**
                         * sendFetchCollaborationDataRequest :used for shared Highlighted and Note database update
                         *//*
                       ...
                       *//* if (array.length() > 0) {
                        ...
                        if (accountType.equalsIgnoreCase(Constants.STUDENT) && _iServiceResponse != null) {
                            getTeacherAnnotation(SDKManager.getInstance().getGetLocalBookData(), bookid, accountType, userid, bookVersion, _iServiceResponse);
                        } else {
                                    ...
                                    if (entry.getValue() == 103 && (!exeption.getResponseRequestType().equals(SERVICETYPES.REFRESH_USER_TOKEN))) {
                                        refreshUserToken(UserController.getInstance(mContext).getUserVO().getToken(), exeption.getResponseRequestType().toString(), new IServiceResponseListener() {
                                            ...
                                            public void requestCompleted(IServiceResponse iServiceResponse) {
                                                ...
                                                String token = responseObj.getUserVO().getToken();
                                                long userId = responseObj.getUserVO().getUserID();
                                                com.hurix.database.user.datamodel.UserVO vo = new com.hurix.database.user.datamodel.UserVO();
                                                vo.setUserID(userId);
                                                vo.setToken(token);
            ...
            for (int i = 0; i < bookClassList.length(); i++) {
                ...
                for (int j = 0; j < classjson.length(); j++) {
                    ...
                    if (obj.has(SHARING_SETTING)) {
                        ...
                        classObj.setNoteStudentStudentSharing(SDKManager.getInstance().is_NoteStudentStudentEnable());
                        classObj.setNoteStudentTeacherSharing(SDKManager.getInstance().is_isNoteTeacherStudentEnable());
                        classObj.setNoteTeacherStudentSharing(SDKManager.getInstance().is_isNoteTeacherStudentEnable());
                        classObj.setHighLightStudentStudentSharing(SDKManager.getInstance().is_isHighlightStudentStudentEnable());
                        classObj.setHighLightStudentTeacherSharing(SDKManager.getInstance().is_isHighlightTeacherStudentEnable());
                        classObj.setHighLightTeacherStudentSharing(SDKManager.getInstance().is_isHighlightTeacherStudentEnable());
                    }
    ...
    private void setpentoolpropertys(com.hurix.customui.datamodel.PentoolVO vo, String metadata) {
        try {
    ...
    public boolean sessionexpire(ServiceException response) {
    ...
    private void showSessionExpiredAlert() {
        DialogUtils.showOKAlert(new View(mContext), 1, mContext, mContext.getResources()
                        .getString(R.string.alert_title), mContext.getResources()
    ...
    public void sendFetchCollaborationDataRequest(final long bookid, final long userid, final String bookVersion) {
        final String timestamp = getLastSyncedDateTime(bookid, userid);
        final String _timestamp = Utils.escapeString(timestamp);
        ...
        super.pageTracking(userToken, bookid, pagetrackdata, new IServiceResponseListener() {
            ...
            public void requestCompleted(IServiceResponse response) {
                ...
                if (responseObj.isSuccess()) {
                    ...
                    Log.d("Analytics", "savePageTrackingData"+"Success");
    ...
    public void sendBackgroundServiceOnBookClose(final long bookid, final long userid, final String bookVersion, String pagetrackdata) {
        ...
        mPagetrackdata = pagetrackdata;
        Log.d("Analytics", "mPagetrackdata");

        if (mPagetrackdata != null && !mPagetrackdata.isEmpty()) {
            savePageTrackingData(KitabooSDKModel.getInstance().getUserToken(), bookid, userid, mPagetrackdata);
            Log.d("Analytics", "savePageTrackingData"+mPagetrackdata);
        }
        int ugcDataLength = DatabaseManager.getInstance(mContext).getUGCData(userid, bookid).length();
    ...
    /*@Override
    public void elasticSearchFetchBookByBookIdRequest(long bookId,String bookType, String mUserToken, IServiceResponseListener callbacklistner) {
        super.elasticSearchFetchBookByBookIdRequest(bookId, bookType, mUserToken, callbacklistner);
    }*/

    @Override
    public void applicationUpdate( IServiceResponseListener callbacklistner) {
        super.applicationUpdate(callbacklistner);
    }
}
