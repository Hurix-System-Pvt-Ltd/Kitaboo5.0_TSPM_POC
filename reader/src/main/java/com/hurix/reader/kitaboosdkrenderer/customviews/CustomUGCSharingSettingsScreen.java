...
import com.hurix.reader.kitaboosdkrenderer.customviews.adapters.UGCSharingSettingAdapter;
...
public class CustomUGCSharingSettingsScreen extends LinearLayout implements OnClickListener,
    ...
    private TextView/* mBackBtnArrow,*/ mTitle, mSharetxt, mRecievetxt, /*mClassSelect,*/
            mShareCheckBox, mRecieveCheckBox;
        ...
        DialogUtils.showOKAlert(new View(mContext), 1, mContext, getResources()
                ...
                new OnDialogOkActionListner() {
                    ...
                    public void onOKClick(View v) {
                        ...
                        com.hurix.reader.kitaboosdkrenderer.utils.Utils.startLauncherActivity(mContext);
                        GlobalDataManager
                                .getInstance().setWebViewClosedAfterTokenReceived(false);
                    }
                });
    }
    ...
    public void requestErrorOccured(ServiceException exeption) {
        if (exeption != null) {
            ...
            if (exceptionObj.getInvalidFields() != null && !exceptionObj.getInvalidFields().isEmpty()) {
                ...
                if (UserController.getInstance(getContext()).getUserSettings().getIsAutoLogOffEnabled()
                        && entry.getValue() == 103) {
                    ...
                    if (entry.getValue() == 103 && !exeption.getResponseRequestType().equals(Constants.SERVICETYPES.REFRESH_USER_TOKEN)) {
                        ...
                        kitabooServiceAdapter.refreshUserToken(UserController.getInstance(mContext).getUserVO().getToken(), exeption.getResponseRequestType().toString(), new com.hurix.service.Interface.IServiceResponseListener() {
                            @Override
                            public void requestCompleted(IServiceResponse response) {


                                com.hurix.service.response.RefreshUserTokenResponse responseObj = (RefreshUserTokenResponse) response;
