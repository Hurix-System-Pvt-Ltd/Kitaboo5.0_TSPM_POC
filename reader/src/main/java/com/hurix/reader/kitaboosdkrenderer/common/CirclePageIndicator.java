...
import com.hurix.reader.kitaboosdkrenderer.PageIndicator;
	  ...
	  public void setCentered(boolean centered) {
	    ...
	    public int getCirclePosition(float x , float y)
	    {
	        for (int i = 0; i < count; i++)
	        {
	        	if(mOrientation == HORIZONTAL)
	        	{
	        		if(xCoord.get(i)+mRadius >= x &&  xCoord.get(i)-mRadius <= x)
	        		{
	        			return i;
	        		}
	        	}
	        	...
	        	{
	        		if( yCoord.get(i)+mRadius >= y &&  yCoord.get(i)-mRadius <= y )
	        		{
	        			return i;	
	        		}
	        	}
	    ...
	    public boolean onTouchEvent(MotionEvent ev) {
	        ...
	        switch (action) {
	            case MotionEvent.ACTION_DOWN:
	                ...
	                setCurrentItem(getCirclePosition(ev.getRawX(), ev.getRawY()));
	                break;
	            ...
	            case MotionEvent.ACTION_UP:
	                if (!mIsDragging) {
	                    ...
	                    final int count1 = mViewPager.getAdapter().getCount();
	                    if ((mCurrentPage > 0) && (ev.getX() < halfWidth - sixthWidth)) {
	                        if (action != MotionEvent.ACTION_CANCEL) {
	                            mViewPager.setCurrentItem(mCurrentPage - 1);
	                        }
	                        return true;
	                    } else if ((mCurrentPage < count1 - 1) && (ev.getX() > halfWidth + sixthWidth)) {
	                        if (action != MotionEvent.ACTION_CANCEL) {
	                            mViewPager.setCurrentItem(mCurrentPage + 1);
	                        }
	                        return true;
